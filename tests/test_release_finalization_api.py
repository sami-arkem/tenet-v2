...
def test_release_finalization_api_flow():
    create_response = client.post(
        ...
        json={
            ...
            "export": {
                ...
                "package_name": "initial_release_pkg_api",
            },
        },
    )
    ...
    assert client.post(f"/v1/control-coverage/audits/{run_id}/build").status_code == 200
    assert client.post(f"/v1/audit-dossier/audits/{run_id}/build").status_code == 200
    assert client.post(
    ...
    assert client.post(f"/v1/release-gate/audits/{run_id}/build").status_code == 200

    finalize_response = client.post(
        f"/v1/release-finalization/audits/{run_id}/finalize",
        json={
            "package_name": "released_pkg_api_001",
            "package_version": "v1",
            "include_model_augmentation": True,
        },
    )
    assert finalize_response.status_code == 200
    data = finalize_response.json()["data"]
    assert data["release_ready"] is True
    assert data["release_status"] == "FINALIZED"
    assert data["released_package"]["verification"]["all_ok"] is True
    assert data["composed_report_summary"]["report_source"] == "composed_report_bundle"

    get_response = client.get(f"/v1/release-finalization/audits/{run_id}")
    assert get_response.status_code == 200
    assert get_response.json()["data"]["run_id"] == run_id

    markdown_response = client.get(f"/v1/release-finalization/audits/{run_id}/markdown")
    assert markdown_response.status_code == 200
    assert "Report source: composed_report_bundle" in markdown_response.json()["data"]["markdown"]
