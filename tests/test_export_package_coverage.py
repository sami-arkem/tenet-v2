...
def test_export_package_writes_control_coverage(tmp_path: Path):
    out = export_audit_package(
        payload=make_payload(),
        export_root=tmp_path,
        package_name="run_export_cov_001_package",
    )
    package_dir = Path(out["package_paths"]["package_dir"])
    assert (package_dir / "control_coverage.json").exists()

    manifest = json.loads((package_dir / "manifest.json").read_text(encoding="utf-8"))
    assert manifest["control_coverage_present"] is True
