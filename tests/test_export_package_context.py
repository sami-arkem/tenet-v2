...
def make_payload():
    return {
        ...
        "audit_context_pack": {
            ...
            "questions": [{"question": "What evidence supports monitoring?", "matches": []}],
        },
    }


def test_export_package_writes_context_pack(tmp_path: Path):
    out = export_audit_package(
        payload=make_payload(),
        export_root=tmp_path,
        package_name="run_export_ctx_001_package",
    )
    package_dir = Path(out["package_paths"]["package_dir"])
    assert (package_dir / "audit_context_pack.json").exists()

    manifest = json.loads((package_dir / "manifest.json").read_text(encoding="utf-8"))
    assert manifest["audit_context_pack_present"] is True
