...
def test_audit_dossier_api_flow():
    ...
    build_response = client.post(f"/v1/audit-dossier/audits/{run_id}/build")
    assert build_response.status_code == 200
    data = build_response.json()["data"]
    assert data["deterministic_authoritative"] is True
    assert data["control_count"] == 1
    assert len(data["controls"][0]["evidence_citations"]) >= 1

    get_response = client.get(f"/v1/audit-dossier/audits/{run_id}")
    assert get_response.status_code == 200
    assert get_response.json()["data"]["run_id"] == run_id

    md_response = client.get(f"/v1/audit-dossier/audits/{run_id}/markdown")
    assert md_response.status_code == 200
    assert "Deterministic Audit Dossier" in md_response.json()["data"]["markdown"]
